    ...
    <div style={{position:"relative",height:"100vh",overflow:"hidden",background:"#000"}}>
      ...
      <div style={{position:"relative",zIndex:2,height:"100%",display:"flex",
        ...
        <div style={{marginBottom:8,animation:"fadeUp 0.8s ease 0.2s both"}}>
          ...
          <h1 style={{fontSize:isMobile?18:28,fontWeight:900,color:"#ffffff",lineHeight:1.2,margin:0,
            fontFamily:"Georgia, serif",textShadow:"0 4px 20px rgba(0,0,0,0.3)",
            maxWidth:isMobile?300:600}}>
            {lang==="ar"?"AI-Powered Analysis of Tourists Opinions on Saudi Attractions":"AI-Powered Analysis of Tourists Opinions on Saudi Attractions"}
          </h1>
...
export default function App(){
        ...
        if(!placesSnap.empty){
          ...
          for(const [cityId,cityPlaces] of Object.entries(DEFAULT_PLACES)){
            for(const place of cityPlaces){
              await setDoc(doc(db,"places",place.id),{ar:place.ar,en:place.en,plusCode:place.plusCode,cityId});
            }
          }
          setPlaces(DEFAULT_PLACES);
        }
        setDbReady(true);
      } catch(e){ console.error("Firebase error:",e); setDbReady(true); }
    }
    loadData();
  },[]);
  ...
  async function addPlace(){
    ...
    await setDoc(doc(db,"places",place.id),{ar:place.ar,en:place.en,plusCode:place.plusCode,cityId:selectedCity.id});
